let data={sections:[]};
const FALLBACK="en",SUPPORTED=["ar","en","es","fr","de","pt","it","tr","nl"],RTL=new Set(["ar","fa","ur","he"]);
const labels={ar:{layan:"تحدث مع ليان",search:"شو الخدمة اللي عم تدور عليها؟",hero:"شو بدك تبني أو تطوّر؟",heroSub:"Company AI تبني مواقع وتطبيقات وأنظمة ذكاء اصطناعي وحلول نمو للأعمال.",explore:"استكشف الخدمات",text:"دردشة نصية مع ليان",voice:"محادثة صوتية مع ليان",notSure:"مو متأكد شو تختار؟ اسأل ليان.",details:"عرض التفاصيل",start:"ابدأ الآن",back:"رجوع",online:"متصلة الآن",send:"إرسال",type:"اكتب رسالتك…"},en:{layan:"Talk to Layan",search:"What service are you looking for?",hero:"What do you want to build or improve?",heroSub:"Company AI builds websites, apps, AI systems and growth solutions for businesses.",explore:"Explore services",text:"Text chat with Layan",voice:"Voice chat with Layan",notSure:"Not sure what to choose? Ask Layan.",details:"View details",start:"Start now",back:"Back",online:"Online",send:"Send",type:"Type your message…"},es:{layan:"Hablar con Layan",search:"¿Qué servicio buscas?",hero:"¿Qué quieres crear o mejorar?",heroSub:"Company AI crea sitios web, aplicaciones, sistemas de IA y soluciones de crecimiento.",explore:"Explorar servicios",text:"Chat de texto con Layan",voice:"Chat de voz con Layan",notSure:"¿No sabes qué elegir? Pregunta a Layan.",details:"Ver detalles",start:"Empezar",back:"Volver",online:"En línea",send:"Enviar",type:"Escribe tu mensaje…"},fr:{layan:"Parler avec Layan",search:"Quel service recherchez-vous ?",hero:"Que voulez-vous créer ou améliorer ?",heroSub:"Company AI crée des sites, applications, systèmes IA et solutions de croissance.",explore:"Découvrir les services",text:"Chat texte avec Layan",voice:"Chat vocal avec Layan",notSure:"Vous ne savez pas quoi choisir ? Demandez à Layan.",details:"Voir les détails",start:"Commencer",back:"Retour",online:"En ligne",send:"Envoyer",type:"Écrivez votre message…"},de:{layan:"Mit Layan sprechen",search:"Welche Leistung suchen Sie?",hero:"Was möchten Sie bauen oder verbessern?",heroSub:"Company AI entwickelt Websites, Apps, KI-Systeme und Wachstumslösungen.",explore:"Leistungen entdecken",text:"Textchat mit Layan",voice:"Sprachchat mit Layan",notSure:"Unsicher? Fragen Sie Layan.",details:"Details ansehen",start:"Starten",back:"Zurück",online:"Online",send:"Senden",type:"Nachricht schreiben…"},pt:{layan:"Falar com Layan",search:"Que serviço procura?",hero:"O que você quer criar ou melhorar?",heroSub:"A Company AI cria sites, apps, sistemas de IA e soluções de crescimento.",explore:"Explorar serviços",text:"Chat de texto com Layan",voice:"Chat de voz com Layan",notSure:"Não sabe o que escolher? Pergunte à Layan.",details:"Ver detalhes",start:"Começar",back:"Voltar",online:"Online",send:"Enviar",type:"Digite sua mensagem…"}};
function detectLanguage(){for(const x of navigator.languages||[navigator.language]){const c=String(x||"").toLowerCase().split("-")[0];if(SUPPORTED.includes(c))return c}return FALLBACK}
function setDocumentLanguage(c){document.documentElement.lang=c;document.documentElement.dir=RTL.has(c)?"rtl":"ltr"}
function t(c,k){return labels[c]?.[k]||labels[FALLBACK][k]||k}
function supportedLanguages(){return SUPPORTED.slice()}
let lang=localStorage.getItem("company-ai-language")||detectLanguage();if(!SUPPORTED.includes(lang))lang="en";
const layanState={history:[]};
async function getLayanConfig(){const r=await fetch("/api/sales-agent/public/config",{cache:"no-store"});if(!r.ok)throw Error("Layan service unavailable");return r.json()}
async function chat(message,language){const c=await getLayanConfig();const r=await fetch("/api/sales-agent/"+c.build_id+"/conversation",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message,language,channel:"text",assistant:"layan",history:layanState.history.slice(-6)})});const d=await r.json();if(!r.ok)throw Error(d.detail||"Layan service unavailable");const reply=String(d.reply||"").trim();layanState.history.push({role:"user",text:message},{role:"assistant",text:reply});return{reply,language:d.language||language}}
function resetChat(){layanState.history=[]}
function speak(text,language){if(!("speechSynthesis"in window))return;window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(text);u.lang=language==="ar"?"ar-LB":language+"-"+(language==="en"?"US":language==="fr"?"FR":language==="de"?"DE":language==="es"?"ES":"");u.rate=.96;window.speechSynthesis.speak(u)}
async function startVoice(s,stateEl,sub,textEl,start){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(!SR){stateEl.textContent="Voice input is not supported in this browser.";sub.textContent="You can use text chat instead.";return}const rec=new SR();window.__layanRec=rec;rec.lang=lang==="ar"?"ar-LB":lang;rec.interimResults=false;rec.continuous=false;stateEl.textContent="Listening…";sub.textContent="Speak naturally.";start.disabled=true;let timer=null;rec.onresult=async e=>{clearTimeout(timer);const msg=e.results[0][0].transcript.trim();textEl.textContent=msg;stateEl.textContent="Layan is thinking…";try{const r=await chat(msg,lang);textEl.textContent=r.reply;stateEl.textContent="Layan is speaking…";speak(r.reply,lang);sub.textContent="You can speak again when she finishes."}catch(err){stateEl.textContent="Something went wrong.";sub.textContent=err.message}finally{start.disabled=false}};rec.onerror=e=>{clearTimeout(timer);stateEl.textContent="Voice input stopped.";sub.textContent=e.error||"Please try again.";start.disabled=false};rec.onend=()=>{clearTimeout(timer);if(start.disabled)start.disabled=false};rec.start();timer=setTimeout(()=>{try{rec.stop()}catch{}},15000)}
...
function bind(){
 ...
 document.querySelectorAll("[data-academy]").forEach(b=>b.onclick=async()=>{
  try{
    const {openAcademy}=await import("./academy.js");
    return openAcademy({lang,$,esc,closeDrawer,openChat});
