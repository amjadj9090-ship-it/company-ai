/* Company AI — Layan Realtime v14 — server-proxied native WebRTC */
(function(){
'use strict';
...
async function start(){if(S.running||S.busy)return;S.busy=true;S.running=true;set('جاري الاتصال…','عم نجهّز اتصال WebRTC آمن عبر خادم Company AI…');try{
 ...
 var answerRes=await fetch(API+'/api/voice-avatar/realtime-call',{method:'POST',headers:{'Content-Type':'application/sdp','Accept':'application/sdp'},body:local.sdp});
 var answer=await answerRes.text();if(!answerRes.ok)throw Error('خادم الصوت أعاد HTTP '+answerRes.status+': '+answer.slice(0,500));
 if(!answer.trim().startsWith('v='))throw Error('خادم الصوت لم يرجع SDP صالحاً');
 await pc.setRemoteDescription({type:'answer',sdp:answer});
 set('لايف — احكي مع ليان','المحادثة الصوتية مفتوحة. الميكروفون شغّال بشكل مستمر.');
 }catch(e){console.error('Layan realtime v14 error',e);set('تعذر تشغيل المحادثة الصوتية',e&&e.message?e.message:'خطأ غير معروف.');S.running=false;cleanup()}finally{S.busy=false}}
